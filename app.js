...
const app = (() => {
  ...
  function renderRepertoire() {
    ...
    const f = currentFilter === 'all' ? repertoireData.map((p, i) => ({ ...p, _idx: i })) : repertoireData.map((p, i) => ({ ...p, _idx: i })).filter(p => p.phase === currentFilter);
    f.forEach((piece, displayIdx) => {
      const idx = piece._idx;
      const progress = state.pieceProgress[piece.name] || 0;
      const progressLabels = ['未开始', '初学', '分手熟练', '合手流畅', '背谱中', '完整演奏'];
      const progressColors = ['bg-piano-600', 'bg-yellow-500/40', 'bg-yellow-500/60', 'bg-accent/60', 'bg-accent/80', 'bg-green-500'];
      const progressPct = (progress / 5) * 100;

      const el = document.createElement('div');
      el.className = `card rounded-xl overflow-hidden fade-in-delay-${Math.min(displayIdx, 3)} cursor-pointer`;
      el.onclick = () => app.openPieceDetail(idx);
        ...
        <div class="border-l-4 ${phaseColor} p-4">
          <div class="flex items-start justify-between gap-3 mb-2">
            ...
            <div class="text-right shrink-0">
              <div class="text-xs font-medium ${progress > 0 ? 'text-accent' : 'text-piano-500'}">${progressLabels[progress]}</div>
              <div class="text-[10px] text-piano-500">${progress}/5</div>
            </div>
          </div>

          <!-- Progress Bar -->
          <div class="flex items-center gap-2 mb-3">
            <div class="flex-1 h-1.5 bg-piano-700 rounded-full overflow-hidden">
              <div class="h-full ${progressColors[progress]} rounded-full transition-all duration-500" style="width: ${progressPct}%"></div>
            </div>
          </div>
  ...
  // ========== PIECE DETAIL & AI REPERTOIRE ASSISTANT ==========
  function openPieceDetail(idx) {
    const piece = repertoireData[idx];
    if (!piece) return;
    const progress = state.pieceProgress[piece.name] || 0;
    const progressLabels = ['未开始', '初学', '分手熟练', '合手流畅', '背谱中', '完整演奏'];
    const phaseLabel = { awakening: '🌱 唤醒期', rebuilding: '🔧 重建期', consolidating: '🏗️ 巩固期', advancing: '🚀 进阶期', reshaping: '👑 重塑期' }[piece.phase] || piece.phase;

    document.getElementById('pd-title').textContent = piece.name;
    document.getElementById('pd-composer').textContent = piece.composer;
    document.getElementById('pd-phase').textContent = phaseLabel;
    document.getElementById('pd-period').textContent = piece.period;
    document.getElementById('pd-difficulty').textContent = piece.difficulty;
    document.getElementById('pd-time').textContent = piece.timeEstimate;
    document.getElementById('pd-focus').textContent = piece.focus;
    document.getElementById('pd-pitfall').textContent = piece.pitfall;
    document.getElementById('pd-check').textContent = piece.check;
    document.getElementById('pd-tags').innerHTML = piece.tags.map(t => `<span class="px-2 py-0.5 rounded-full bg-piano-700/50 text-piano-300 text-xs">${t}</span>`).join('');

    // Render progress buttons
    const pb = document.getElementById('pd-progress-buttons');
    pb.innerHTML = progressLabels.map((label, i) => `
      <button onclick="app.updatePieceProgress(${idx}, ${i})" class="flex-1 py-2 rounded-lg text-xs font-medium transition-all ${i === progress ? 'bg-accent text-piano-900' : 'bg-piano-700 text-piano-300 hover:bg-piano-600'}">${label}</button>
    `).join('');

    // IMSLP link
    const searchQ = encodeURIComponent(piece.name.replace(/·/g, ' '));
    document.getElementById('pd-imslp').href = `https://imslp.org/wiki/Special:Search?search=${searchQ}`;

    document.getElementById('piece-detail-modal').classList.remove('hidden');
  }
  function closePieceDetail() {
    document.getElementById('piece-detail-modal').classList.add('hidden');
  }
  function updatePieceProgress(idx, status) {
    const piece = repertoireData[idx];
    if (!piece) return;
    state.pieceProgress[piece.name] = status;
    save();
    renderRepertoire();
    openPieceDetail(idx); // Refresh modal
  }

  // AI Repertoire Assistant
  function openRepertoireAI() {
    document.getElementById('repertoire-ai-modal').classList.remove('hidden');
    document.getElementById('rai-input').focus();
  }
  function closeRepertoireAI() {
    document.getElementById('repertoire-ai-modal').classList.add('hidden');
  }
  function askRepertoireAI(query) {
    const q = (query || document.getElementById('rai-input').value || '').toLowerCase().trim();
    if (!q) return;
    const out = document.getElementById('rai-messages');

    // User message
    const userEl = document.createElement('div');
    userEl.className = 'flex justify-end mb-3';
    userEl.innerHTML = `<div class="max-w-[80%] bg-accent/20 rounded-xl px-4 py-2 text-sm text-white">${escapeHtml(query || document.getElementById('rai-input').value)}</div>`;
    ...
    // Parse query
    const phaseKeywords = {
      awakening: ['唤醒', '入门', '开始', '基础', '生疏', '简单'],
      rebuilding: ['重建', '恢复', '练习', '技巧', '哈农', '车尔尼'],
      consolidating: ['巩固', '中等', '巴赫', '小奏鸣曲'],
      advancing: ['进阶', '肖邦', '李斯特', '高级', '困难'],
      reshaping: ['重塑', '演奏', '演出', '大师', '终极']
    };
    const composerKeywords = {
      Bach: ['巴赫', 'bwv', 'bach'],
      Mozart: ['莫扎特', 'mozart'],
      Beethoven: ['贝多芬', 'beethoven'],
      Chopin: ['肖邦', 'chopin'],
      Schubert: ['舒伯特', 'schubert'],
      Debussy: ['德彪西', 'debussy'],
      Rachmaninoff: ['拉赫', 'rachmaninoff'],
      Liszt: ['李斯特', 'liszt'],
      Czerny: ['车尔尼', 'czerny'],
      Hanon: ['哈农', 'hanon'],
      Burgmuller: ['布格缪勒', 'burgmuller'],
      Clementi: ['克莱门蒂', 'clementi'],
      Kuhlau: ['库劳', 'kuhlau']
    };
    const techniqueKeywords = {
      scale: ['音阶', '琶音', '五指', '手指', '跑动'],
      chord: ['和弦', '八度', '和弦'],
      articulation: ['断奏', '连奏', '触键', 'articulation'],
      polyphony: ['复调', '对位', '声部', '多声部'],
      expression: ['表现', '音色', 'rubato', '音乐性', '歌唱'],
      rhythm: ['节奏', '切分', '拍子']
    };

    let matchedPhase = null, matchedComposer = null, matchedTech = null;
    for (const [ph, keys] of Object.entries(phaseKeywords)) {
      if (keys.some(k => q.includes(k))) { matchedPhase = ph; break; }
    }
    for (const [comp, keys] of Object.entries(composerKeywords)) {
      if (keys.some(k => q.includes(k))) { matchedComposer = comp; break; }
    }
    for (const [tech, keys] of Object.entries(techniqueKeywords)) {
      if (keys.some(k => q.includes(k))) { matchedTech = tech; break; }
    }

    // Score and filter
    let results = repertoireData.map((p, i) => {
      let score = 0;
      if (matchedPhase && p.phase === matchedPhase) score += 10;
      if (matchedComposer && p.composer === matchedComposer) score += 8;
      if (matchedTech) {
        const techMap = { scale: ['音阶', '跑动', '手指'], chord: ['和弦', '八度'], articulation: ['触键', '断奏'], polyphony: ['复调', '声部'], expression: ['音色', '歌唱', '表现'], rhythm: ['节奏'] };
        if (techMap[matchedTech] && techMap[matchedTech].some(k => (p.focus + p.tags.join(' ')).includes(k))) score += 5;
      }
      // Text match
      const text = (p.name + p.focus + p.composer + p.tags.join(' ')).toLowerCase();
      if (text.includes(q)) score += 15;
      // Phase ordering: prefer user's current level
      const phaseOrder = { awakening: 1, rebuilding: 2, consolidating: 3, advancing: 4, reshaping: 5 };
      score += (phaseOrder[p.phase] || 3) * 0.5;
      return { ...p, _idx: i, score };
    }).filter(p => p.score > 0).sort((a, b) => b.score - a.score).slice(0, 6);

    // If no keyword match, suggest based on user's current week
    if (results.length === 0) {
      const week = state.currentWeek || 1;
      const suggestedPhase = week <= 2 ? 'awakening' : week <= 4 ? 'rebuilding' : week <= 7 ? 'consolidating' : week <= 10 ? 'advancing' : 'reshaping';
      results = repertoireData.map((p, i) => ({ ...p, _idx: i, score: p.phase === suggestedPhase ? 10 : Math.abs((phaseOrder[p.phase] || 3) - phaseOrder[suggestedPhase]) }))
        .sort((a, b) => b.score - a.score).slice(0, 5);
    }

    // Build response
    const phaseOrder = { awakening: 1, rebuilding: 2, consolidating: 3, advancing: 4, reshaping: 5 };
    const reason = matchedPhase ? `根据你询问的「${matchedPhase}」阶段` : matchedComposer ? `找到 ${matchedComposer} 的作品` : matchedTech ? `针对「${matchedTech}」技巧` : `基于你当前第 ${state.currentWeek || 1} 周的复健进度`;

    let html = `<div class="text-sm text-piano-200 mb-3">🤖 ${reason}，为你推荐以下曲目：</div><div class="space-y-2">`;
    results.forEach(r => {
      const phLabel = { awakening: '🌱', rebuilding: '🔧', consolidating: '🏗️', advancing: '🚀', reshaping: '👑' }[r.phase] || '';
      const searchQ = encodeURIComponent(r.name.replace(/·/g, ' '));
      html += `
        <div class="card rounded-xl p-3 cursor-pointer hover:border-accent/30" onclick="app.closeRepertoireAI(); app.switchTab('repertoire'); app.openPieceDetail(${r._idx})">
          <div class="flex items-center justify-between">
            <div class="flex-1 min-w-0">
              <div class="font-medium text-white text-sm">${phLabel} ${r.name}</div>
              <div class="text-xs text-piano-400 mt-0.5">${r.focus}</div>
            </div>
            <a href="https://imslp.org/wiki/Special:Search?search=${searchQ}" target="_blank" onclick="event.stopPropagation()" class="ml-2 px-2 py-1 rounded-lg bg-piano-700 text-xs text-accent hover:bg-piano-600">📄 找谱</a>
          </div>
        </div>
      `;
    });
    ...
    aiEl.className = 'flex justify-start mb-3';
    aiEl.innerHTML = `<div class="max-w-[90%] ai-card rounded-xl px-4 py-3 text-sm">${html}</div>`;
  ...
  return {
    ...
    dismissAdaptiveTip,
    // Tier 2 exports
    openPieceDetail, closePieceDetail, updatePieceProgress,
    openRepertoireAI, closeRepertoireAI, askRepertoireAI
  };
})();
